...
# In-memory storage for verification codes and password reset codes (in production, use Redis or database)
verification_codes = {}
...
def forgot_password():
    try:
        ...
        # Send OTP email
        msg = Message(
            subject='SmartCal - Password Reset OTP',
            recipients=[email],
            body=f'''
Hello!

We received a request to reset your password for your SmartCal account. Please use the following verification code to reset your password:
...
<html>
<body>
    <h2>SmartCal - Password Reset</h2>
    <p>Hello!</p>
    <p>We received a request to reset your password for your SmartCal account. Please use the following verification code to reset your password:</p>
    <h3 style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; text-align: center; font-size: 24px; letter-spacing: 5px;">{otp}</h3>
    <p><strong>This code will expire in 5 minutes.</strong></p>
...
@app.route('/users/verify-reset-otp', methods=['POST'])
def verify_reset_otp():
    try:
        ...
        if email not in password_reset_codes:
            return jsonify({'detail': 'No OTP found for this email or OTP has expired'}), 400
        ...
        # OTP is valid, but don't delete it yet as we need it for the password reset step
...
def reset_password():
    try:
        ...
        if email not in password_reset_codes or not password_reset_codes[email].get('verified'):
            return jsonify({'detail': 'Please verify your OTP first'}), 400
        
        # Get user ID from stored data
        user_id = password_reset_codes[email]['user_id']
